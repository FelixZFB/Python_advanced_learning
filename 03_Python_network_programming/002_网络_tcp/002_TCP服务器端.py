...
def main():
    ...
    # 2. tcp服务器端是绑定ip地址和port端口，绑定本机IP
    addr = ('127.0.0.1', 8080)
...
# 打开网络调试助手，选择TCP client，主机地址改成上面代码中绑定的地址和端口，即连接上面的服务器端
...
# client_addr的ip地址就是服务器端的IP地址，因为现在都是在同一个电脑，但是端口不一样
