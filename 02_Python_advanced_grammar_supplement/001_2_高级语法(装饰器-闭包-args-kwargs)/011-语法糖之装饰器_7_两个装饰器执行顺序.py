...
# 7. 然后又返回继续执行@deco2，把新的func_a(@deco1装饰之后的func_a)，即deco1.wrapper作为参数传个deco2函数
# 8. 执行deco2函数，从而打印出：002装饰器开始对函数进行装饰，
# 9. 然后@deco2内部函数开始对新func_a=deco1.wrapper进行装饰，装饰之后，deco2返回值为内函数wrapper，
# 10. 此时，新新函数func_a已经又指向了deco2的内函数wrapper的地址，即新新func_a = deco2.wrapper

# 第一次装饰后func_a指向了deco1.wrapper， 第二次装饰后deco1.wrapper指向了deco2.wrapper
# 即最终func_a指向了deco2.wrapper函数，即func_a--->deco1.wrapper--->deco2.wrapper
...
# 15. 语句中的func()执行的时候寻找到的deco2传入的func,传入的是deco1.wrapper
#     即func_a的上一级地址deco1.wrapper(deco2.wrapper地址刚刚上面已经使用了)
#     即func()此时指向的是deco1.wrapper地址，此时就运行deco1.wrapper
#     先打印出001装饰器内函数开始执行，然后返回一个值" (001) " + func() + " (001) ",传给deco2 return中的func()
# 16. 上面14返回值中的func()即deco1.wrapper运行后的返回值：" (001) " + func() + " (001) "
# 17. 因此deco2.wrapper函数返回的实际是：" [002] " + " (001) " + func() + " (001) " + " [002] "

# 18. 此时语句中的func()才是指向的原始函数func_a的地址，执行原始函数func_a,打印出：...003原始函数终于执行了...
# 17. 原始函数func的返回值是："Hello Python Decorator"
# 18. 因此最终func_a函数运行后的返回值是：" [002] " + " (001) " + "Hello Python Decorator" + " (001) " + " [002] "
# 19. 最后执行print(ret)即打印出经过装饰之后func_a()函数执行完毕后的返回值


# 总之，装饰器装饰的时候，从最靠近原始函数开始进行装饰，逐级向外进行(外函数)，
# 所有装饰函数装饰结束后，再执行原始函数，此时原始函数指向的是最后一个装饰器内函数的地址，
# 内函数又从最外面的装饰器开始执行，逐级向里面执行（内函数），所有内函数开始执行完了，最后开始执行原始函数下的代码
# 先里后外装饰，再外后里执行，最后原始执行

# 装饰时：func_a--->deco1.wrapper--->deco2.wrapper
# 执行时：deco2.wrapper--->deco1.wrapper--->func_a
